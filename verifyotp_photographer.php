        ...
        <main>
            <div class="otp-card">
                <?php
                    ...
                    $query = $connection->prepare("SELECT * FROM photographer WHERE email = ?");
                        ...
                        if($queryResult->num_rows > 0){
                            ...
                            ?>
                                ...
                                <?php
                                    if(isset($_POST['submit'])){
                                        ...
                                        $query1 = $connection->prepare ("SELECT * FROM photographer where email = ?");
                                        ...
                                        if ($query1Result-> num_rows > 0) {
                                            ...
                                            if($dbOTP != $inputOTP){
                                                ...
                                                $otp = 0;
                                                $expire_time = null;
                                                $updateQuery = $connection->prepare("UPDATE photographer set otp = ?, expire_time = ? WHERE email = ?");
                                                $updateQuery->bind_param("iss",$otp, $expire_time, $email);
                                                if ($updateQuery->execute()){
                                                    echo "<script>window.location.href = 'photographer/photographer_dashboard.php'; alert('Successfully registered');</script>";
                                                    $updateQuery->close();
                                                    exit();
                                                } else {
                            ...
                            <?php
                                if(isset($_POST['resend'])){
                                    $resendQuery = $connection->prepare("UPDATE photographer SET otp = ?, expire_time = ? WHERE email = ?");
