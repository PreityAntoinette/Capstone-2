<!-- for equipment update -->
<div class="modal-overlay"  id="<?php echo 'equipment'.$id; ?>">
    <div class="modal-container modal-form-size modal-sm">
        <div class="modal-header text-light">
            <h4 class="modal-h4-header">Update Equipment</h4>
            <span class="modal-exit" data-modal-id="<?php echo 'equipment'.$id; ?>">
                <svg
            ...
            <div class="modalContent">
                <form method="POST" enctype="multipart/form-data">
                    <div class="form-group ">
                        <label for="resources">Equipment Name:</label>
                        ...
                        <input class="" id="resources" value="<?php echo $name; ?>" type="text"name="resources_name" required>    
                    </div>
                    <div class="form-group">
                        <label for="imgs">Image <i>(5mb max. size)</i></label>
                        <input value="" onchange='validate(this)' type="file" name="image" id="file" class="file-input" accept="image/*">  
                        <p class="output"></p>
                    </div>
                    <div class="form-group">
                        ...
                        <select name="type">
                            ...
                            <option value="EQUIPMENT" <?php echo ($type=="EQUIPMENT")? 'selected':'';?> >Equipment</option>
                        </select>
                    </div>
...
<?php
	if (isset($_POST['submit_resources'])){
        ...
        if (!empty($_FILES['image']['name'])) {
            ...
            mysqli_query($connection, "UPDATE tbl_resources 
            SET name='$name', image='$renameImg', status='$status', type='$type', quantity = '$quantity'
            WHERE resources_id='$id' AND type='EQUIPMENT'") or die(mysqli_error($connection));
            $jsCode = '
            <script>
           var description = "Resource has been updated.<br> </br> Resource Name: '. ucfirst(strtolower($_POST['resources_name'])) .'<br> Type: '. ucfirst(strtolower($_POST['type'])) .'<br> Status: '.$_POST['status'] .'";
            ...
            mysqli_query($connection, "UPDATE tbl_resources
            SET name='$name', status='$status', type='$type', quantity = '$quantity'
            WHERE resources_id='$id' AND type='EQUIPMENT'") or die(mysqli_error($connection));
            $jsCode = '
            <script>
           var description = "Resource has been updated.<br> </br> Resource Name: '. ucfirst(strtolower($_POST['resources_name'])) .'<br> Type: '. ucfirst(strtolower($_POST['type'])) .'<br> Status: '.$_POST['status'] .'";
...
?>
<!-- for viewing resource image -->
<div class="modal-overlay"  id="<?php echo 'equipmentImg'.$id; ?>">
    <div class="modal-img" >
        <span class="modal-exit" data-modal-id="<?php echo 'equipmentImg'.$id; ?>">
            <svg
                xmlns="http://www.w3.org/2000/svg"
                width="25"
                height="25"
...
<!-- for archiving equipment -->
<div class="modal-overlay"  id="<?php echo 'archive_equip'.$id; ?>">
    <div class="modal-container modal-form-size modal-sm">
        <div class="modal-header text-light">
            ...
            <span class="modal-exit" data-modal-id="<?php echo 'archive_equip'.$id; ?>">
                <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="25"
                    height="25"
