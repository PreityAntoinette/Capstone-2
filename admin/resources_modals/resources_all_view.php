...
<div class="modal-overlay"  id="<?php echo 'resources'.$id; ?>">
    <div class="modal-container modal-form-size modal-sm">
        <div class="modal-header text-light">
            <h4 class="modal-h4-header">Update Resources</h4>
            <span class="modal-exit" data-modal-id="<?php echo 'resources'.$id; ?>">
                <svg
...
<?php
	if (isset($_POST['submitall'])){
        ...
        if (!empty($_FILES['image']['name'])) {
            ...
            mysqli_query($connection, "UPDATE tbl_resources SET name='$name', image='$renameImg', status='$status', type='$type',quantity = '$quantity' WHERE resources_id='$id'") or die(mysqli_error($connection));
            $jsCode = '
            <script>
           var description = "Resource has been updated.<br> </br> Resource Name: '. ucfirst(strtolower($_POST['resources_name'])) .'<br> Type: '. ucfirst(strtolower( $_POST['type'])) .'<br> Status: '.$_POST['status'] .'";
            ...
            mysqli_query($connection, "UPDATE tbl_resources SET name='$name', status='$status' , type='$type', quantity = '$quantity' WHERE resources_id='$id'") or die(mysqli_error($connection));
            $jsCode = '
            <script>
           var description = "Resource has been updated.<br> </br> Resource Name: '. ucfirst(strtolower($_POST['resources_name'])) .'<br> Type: '. ucfirst(strtolower($_POST['type'])) .'<br> Status: '.$_POST['status'] .'";
            document.addEventListener("DOMContentLoaded", function() {
...
?>
<!-- for viewing resources image -->
<div class="modal-overlay"  id="<?php echo 'resourcesImg'.$id; ?>">
    <div class="modal-img" >
        <span class="modal-exit" data-modal-id="<?php echo 'resourcesImg'.$id; ?>">
            <svg
                xmlns="http://www.w3.org/2000/svg"
                width="25"
                height="25"
