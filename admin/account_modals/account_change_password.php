...
<div class="modal-overlay" id="<?php echo 'change_password'.$id; ?>">
     <div class="modal-container modal-form-size" style="max-width: 500px !important">
        <div class="modal-header text-light">
            ...
            <span class="modal-exit" data-modal-id="<?php echo 'change_password'.$id; ?>">
                <svg
        ...
        if (password_verify($old_pass, $pass)) {
            ...
            if (password_verify($new_pass, $pass)){
                ...
                if ($new_pass!=$con_pass){ // if the new password does not match confirm password then display error message
                    ...
                    $insertQuery = $connection->prepare ("UPDATE users SET password = ? WHERE user_id = ?"); 
                    $insertQuery -> bind_param("si", $hashed_new_pass, $id);
                    $insertQuery->execute();
