<!-- for user archiving -->
<div class="modal-overlay"  id="<?php echo 'unarchive_user' . $id;?>">
    <div class="modal-container modal-form-size modal-sm">
        <div class="modal-header text-light">
            ...
            <span class="modal-exit" data-modal-id="<?php echo 'unarchive_user' . $id;?>">
                <svg
        ...
        <div class="modal-body">
            ...
            <form method="POST" action="user_list.php?activeTable=table2" enctype="multipart/form-data">
                <p class="justify-content-center">Move &nbsp;<span class="text-danger"><?php echo " " . ucfirst($fname). " " . ucfirst($lname);?> 's</span> &nbsp; account to unarchive?</p>
                    ...
                    <div class="modal-footer">
                        <button type="submit" name="unarchived_user" class="btn btn-warning text-dark"  onsubmit='return validate()'>Confirm</button>
                        <button class="btn close" data-modal-id="<?php echo 'unarchive_user' . $id;?>">Cancel</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    <?php
    // Assuming you have a variable $userRole that holds the user role
    $role = 'user';  // Replace this with the actual variable that holds the user role

    if ($role == 'user' && isset($_POST['unarchived_user'])) {
        $status = 1;
        ...
        $updateStatus->bind_param("ii" , $status, $user_id);
        if ($updateStatus->execute()) {
            $jsCode = '
            <script>
                var description = "Archived borrower successfully! <br> </br> Name: '. ucfirst($fname) . " ". ucfirst($lname). '";
